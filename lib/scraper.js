...
const GameReqsScraper=()=>{
  const reqScraper=async()=>{
    ...
    await browser.close();
    console.log(reqData);
    const reqData1=reqData.slice(0,10);
          ...
          if(div.querySelector(".game_area_sys_req_leftCol") !== null)
          {
            ...
            li.forEach(element => {
              if(element.querySelector("strong")!==null)
              {
                ...
                {
                  const cpuname=element.textContent.slice(10).trim();
                  let index=[];
                  index[0] = cpuname.indexOf(" or ") === -1 ? 1e9 : cpuname.indexOf(" or ");
                  index[1] = cpuname.indexOf("/") === -1 ? 1e9 : cpuname.indexOf("/");
                  index[2] = cpuname.indexOf("|") === -1 ? 1e9 : cpuname.indexOf("|");
                  index[3] = cpuname.indexOf(",") === -1 ? 1e9 : cpuname.indexOf(",");
                  let min=Math.min(index[0],Math.min(index[1],Math.min(index[2],index[3])));
                  if(min!==1e9)
                    obj.cpu=cpuname.slice(0,min).trim();
                  else
                    obj.cpu=cpuname;
                }
                else if(type.innerHTML === "Graphics:")
                {
                  const gpuname=element.textContent.slice(9).trim();
                  let index=[];
                  index[0] = gpuname.indexOf(" or ") === -1 ? 1e9 : gpuname.indexOf(" or ");
                  index[1] = gpuname.indexOf("/") === -1 ? 1e9 : gpuname.indexOf("/");
                  index[2] = gpuname.indexOf("|") === -1 ? 1e9 : gpuname.indexOf("|");
                  index[3] = gpuname.indexOf(",") === -1 ? 1e9 : gpuname.indexOf(",");
                  let min=Math.min(index[0],Math.min(index[1],Math.min(index[2],index[3])));
                  if(min!==1e9)
                    obj.gpu=gpuname.slice(0,min).trim();
                  else
                    obj.gpu=gpuname;
                }
